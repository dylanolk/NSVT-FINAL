...
def frequency_modulator(file, shift):
    #bytes_wav = bytes()
...
@app.route("/shiftFrequencyUp", methods=['POST', 'GET'])
def shiftFrequencyUp():
    print(request.files)
    ...
    #spf = wave.open(filename, 'rb')
    #spf = wave.open("temp.wav", 'rb')
    spf = wave.open(request.files['file'])
    RATE=spf.getframerate()
    signal = spf.readframes(-1)
    wf = wave.open('new.wav', 'wb')
    wf.setnchannels(CHANNELS)
...
@app.route("/shiftFrequencyDown", methods=['POST', 'GET'])
def shiftFrequencyDown():
    print(request.files)
    if 'file' in request.files:
        print('true')
        try:
...
@app.route("/roboticize", methods=['POST', 'GET'])
def roboticize():
    #while(input_filename[-4:] != ".wav"):
            #file = input("file name must end in '.wav' ")

    audio = AudioSegment.from_file(request.files['file'])

    for i in range (3):
        audio = audio.overlay(audio, position = 5)        

    audio.export("robot.wav", format='wav') #export mixed  audio file

    wav_string = base64.b64encode(open("robot.wav" + input_filename).read())
    array = wav_to_array("robot.wav")
