...
class FolderDialog extends React.Component{
  ...
  changeHandler(event) {
	...
	async postSelectedFile(selectedFile) {
		...
		var temparray=[]
		await axios.post("http://127.0.0.1:5000/process", formData).then(function(response){
			...
			<div class="cards">
			  <div>
				...
				{!this.state.isFilePicked ? (
				  ...
				  <Button title="Convert File" handleSubmission={() =>this.postSelectedFile(this.state.selectedFile)} />
				)}
			  </div>
			</div>
		);
